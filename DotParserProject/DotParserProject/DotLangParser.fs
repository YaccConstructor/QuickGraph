...
let parse (input_file: string) =
    graphs <- ResizeArray.create 7 (new GraphDataContainer ())
    let tokens = 
...
let parsestring (input_graph: string) =
    graphs <- ResizeArray.create 7 (new GraphDataContainer ())
    
    let tokens = 
        let lexbuf = Lexing.LexBuffer<_>.FromString <| input_graph
        seq { while not lexbuf.IsPastEndOfStream do
                  yield DotParserProject.DotLexer.tokenize lexbuf }
...
let VertAndEdgesString s = 
    let graphs = parsestring s
    let main_graph = graphs.[0]
    let mutable lst = []  
    let mutable arr = [||]
    let mutable str = ""
