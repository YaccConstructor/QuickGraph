...
namespace DagShortestPathPlugin
{
    ...
    [Extension]
    public class DagShortestPathPlugin : IAlgorithm
    {
        ...
        private DagShortestPathAlgorithm<GraphXVertex, GraphXTaggedEdge<GraphXVertex, int>> dag;
        ...
        public string Name => "Dag algorithm Plugin";
        public string Author => "Alisa Meteleva";
        public string Description => "This plugin demonstrates how Dag alg works.\n";
        ...
        public void Run(string dotSource)
        {
            ...
            dag = new DagShortestPathAlgorithm<GraphXVertex, GraphXTaggedEdge<GraphXVertex, int>>(graph, x => x.Tag);

            dag.DiscoverVertex += OnDiscoverVertex;
            dag.FinishVertex += OnStartVertex;
            dag.Compute(startVertix);
            ...
            _hasStarted = true;
            _hasFinished = false;
            foreach (var v in dag.VisitedGraph.Vertices)
            {
                distances += "Shortest distance for vertix" + v.ToString() + " equals " + dag.Distances[v].ToString() + "\n";
            }
        }
        ...
        public void NextStep()
        {
            ...
            var shortDistance = dag.Distances[states[currState].v];
