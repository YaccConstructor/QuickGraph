...
namespace YenVisualisation
{
  ...
  public class Yen : IAlgorithm
  {
    ...
    private bool GetYenIterations(Graph graph)
    {
      int numberK;
      bool result = Int32.TryParse(_k.Text, out numberK);
      if (!result || _startVertex.Text.Length != 1 || _finishVertex.Text.Length != 1)
      {
        return false;
      }
      var yenGraph = ToYenInput(graph);
      var yen = new YenShortestPathsAlgorithm<char>(yenGraph, _startVertex.Text.First(), _finishVertex.Text.First(), numberK);
    ...
    {
      if (_startVertex.Text.Length == 0 || _finishVertex.Text.Length == 0)
      {
        MessageBox.Show("Please specify start and finish vertices first!\n");
        return;
      }
      ...
      try
      {
        var res = GetYenIterations(graph);
        if (!res)
        {
      ...
      {
        MessageBox.Show("Sorry! \n Yen algorithm doesn't supporst this graph! \n");
      }
    ...
    public string Description =>
      "Find k shortest ways in directed graph \n";
