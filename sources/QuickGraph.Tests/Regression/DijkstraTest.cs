...
namespace QuickGraph.Tests.Regression {
    ...
    class DijkstraTest {
        ...
        public void Scenario() {
            ...
            using (ObserverUtility.Guard<IDistanceRecorderAlgorithm<string, Edge<string>>>(dijkstra, distObserver))
            using (ObserverUtility.Guard<IVertexPredecessorRecorderAlgorithm<string, Edge<string>>>(dijkstra, predecessorObserver)) 
            {
                ...
                foreach (KeyValuePair<string, int> kvp in distObserver.Distances)
                    Console.WriteLine("Distance from A to node {0} is {1}", kvp.Key, kvp.Value);
